...
server (when receiving requests from outside).
...
If these keys are provided then the buss will try to authenticate
using `login.identity.exchange` method, otherwise the
authentication method will be `login.identity.check`

The keys can be generated by a script, using some JSON Object Signing and Encryption library. For example [jose](https://github.com/panva/jose):
